...
/* 
	Dieses Programm dient zur Simulation des Spieles Pong.
*/
...
struct ball 
{
 ...
 void reset(pad &you, pad &comp, ball &b, bool &started, int &you_score, int &comp_score) 
 {
  ...
  p.Y = height;
  SetConsoleCursorPosition( hOut, p );
  cout << "\n    My score: " << you_score << "\tKi score: " << comp_score << '\n';
 }
...
//---------- Ball startet immer in richtung Spieler. ----------
   b.vx = float(rand() % 3) - 1.5f;
   b.vy = 1;
   comp.vx = 0;
   while ( true ) 
   {
     if ( started ) 
	 {
       if ( GetAsyncKeyState(VK_LEFT) ) // mit Pfeiltasten
	   {
         you.x -= 2;
       } 
